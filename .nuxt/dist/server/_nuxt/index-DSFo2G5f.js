...
import { ref, shallowRef, toRef, getCurrentInstance, onServerPrefetch, unref, computed, toValue, reactive, defineComponent, withAsyncContext, withCtx, createTextVNode, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate } from "vue/server-renderer";
import { hash } from "ohash";
import { getRequestHeaders } from "h3";
import { u as useNuxtApp, a as asyncDataDefaults, c as createError, f as fetchDefaults } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data } = ([__temp, __restore] = withAsyncContext(() => useFetch("/api/data", "$KGEEURg5ou")), __temp = await __temp, __restore(), __temp);
    console.log(data.value);
    const headers = useRequestHeaders(["cookie"]);
    console.log(headers.value);
    const { data: discounts, status } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData(
      "cart-discount",
      async () => {
    ...
    }, "$e8U1oTCjBi")), await __temp, __restore();
    return (_ctx, _push, _parent, _attrs) => {
      const _component_nuxt_link = __nuxt_component_0;
      _push(`<div${ssrRenderAttrs(_attrs)}><div>test页面</div>`);
      ...
      _push(`<div>addTodo</div> Counter: ${ssrInterpolate(unref(counter))} <button>+</button><button>-</button></div>`);
    };
  }
});
...
//# sourceMappingURL=index-DSFo2G5f.js.map
